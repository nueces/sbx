...
<style>
:root {{ --ink:#2e3440;--ink2:#1f2430;--stone:#4c566a;--paper:#f3e8d0;--paper2:#fff7ed;--leather:#7a4a2b;--gold:#d49a3a;--cyan:#88c0d0;--green:#5d8f64;--red:#bf616a;--shadow:#b79b75;--grid:rgba(46,52,64,.035); }}
body:has(#theme:checked) {{ --ink:#f3e8d0;--ink2:#05070d;--stone:#cbd5e1;--paper:#080711;--paper2:#111827;--leather:#f6c453;--gold:#f6c453;--cyan:#88c0d0;--green:#8fbc8f;--red:#f87171;--shadow:#000;--grid:rgba(255,255,255,.045); }}
*{{box-sizing:border-box}} html{{scroll-behavior:smooth}} body{{margin:0;color:var(--ink);background:linear-gradient(90deg,var(--grid) 1px,transparent 1px),linear-gradient(var(--grid) 1px,transparent 1px),var(--paper);background-size:24px 24px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;line-height:1.55}} a{{color:var(--leather)}} code{{font:inherit;font-size:.92em}} .theme{{position:absolute;opacity:0}} .header{{max-width:1240px;margin:auto;padding:28px 20px 0}} nav{{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:14px 18px;background:var(--ink);color:var(--paper2);border:4px solid var(--ink2);box-shadow:6px 6px var(--shadow)}} nav a{{color:var(--paper2);text-decoration:none}} .brand{{font-size:22px;font-weight:900}} .links{{display:flex;gap:16px;flex-wrap:wrap;align-items:center}} label{{cursor:pointer;background:var(--gold);color:#1f2430;padding:3px 9px;border:3px solid var(--paper2);font-weight:900;box-shadow:3px 3px #000}} .hero,main{{max-width:1240px;margin:auto;padding:34px 20px}} .frame{{background:var(--paper2);border:4px solid var(--ink);box-shadow:8px 8px var(--shadow)}} .hero-box{{padding:32px}} .eyebrow{{display:inline-block;background:var(--leather);color:var(--paper2);border:3px solid var(--ink);padding:5px 9px;font-weight:900}} h1,h2,h3{{line-height:1.1;letter-spacing:-.04em}} h1{{font-size:clamp(42px,7vw,82px);margin:16px 0}} h2{{font-size:clamp(30px,4vw,48px);margin:0 0 16px}} h3{{font-size:21px}} .subtitle{{max-width:900px;color:var(--stone);font-size:19px;font-weight:700}} .meta{{color:var(--stone);font-size:13px}} section{{margin:0 0 52px}} .cards{{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}} .card{{padding:20px}} .number{{font-size:35px;font-weight:900;line-height:1}} .card p{{margin:8px 0 0;color:var(--stone)}} .summary{{display:grid;grid-template-columns:1.4fr 1fr;gap:18px}} .panel{{padding:22px}} .callout{{border-left:10px solid var(--red)}} .good{{border-left:10px solid var(--green)}} .table-wrap{{overflow:auto;border:4px solid var(--ink);box-shadow:6px 6px var(--shadow);background:var(--paper2)}} table{{width:100%;border-collapse:collapse;min-width:850px}} th,td{{padding:10px 12px;text-align:left;border-bottom:1px solid color-mix(in srgb,var(--stone) 28%,transparent);vertical-align:top}} th{{position:sticky;top:0;background:var(--ink);color:var(--paper2);font-size:12px;text-transform:uppercase}} td small{{display:block;color:var(--stone)}} tbody tr:hover{{background:color-mix(in srgb,var(--cyan) 18%,transparent)}} .hot-row{{background:color-mix(in srgb,var(--red) 10%,transparent)}} .badge{{display:inline-block;padding:2px 7px;border:2px solid currentColor;font-size:11px;font-weight:900;text-transform:uppercase;white-space:nowrap}} .low{{color:var(--green)}} .moderate{{color:#a56b00}} .high,.very-high,.severe,.critical{{color:var(--red)}} .bar{{display:flex;height:24px;border:3px solid var(--ink);margin:14px 0}} .bar span{{display:grid;place-items:center;color:white;font-size:10px;font-weight:900;min-width:30px}} .bar .a{{background:#5d8f64}} .bar .b{{background:#c08a28}} .bar .c{{background:#bf616a}} ol li{{margin:10px 0}} details{{background:var(--paper2);border:4px solid var(--ink);box-shadow:6px 6px var(--shadow)}} summary{{cursor:pointer;padding:15px 18px;font-weight:900}} details .table-wrap{{border:0;box-shadow:none}} .legend{{display:flex;gap:10px;flex-wrap:wrap}} footer{{padding:22px;background:var(--ink);color:var(--paper2)}} @media(max-width:900px){{.cards,.summary{{grid-template-columns:1fr 1fr}}}} @media(max-width:600px){{.cards,.summary{{grid-template-columns:1fr}}.links a{{display:none}}}}
</style>
</head>
<body>
<input class="theme" id="theme" type="checkbox" aria-label="Use dark theme">
<header class="header"><nav><a class="brand" href="index.html">▣ sbx complexity map</a><div class="links"><a href="index.html">home</a><a href="#overview">overview</a><a href="#modules">modules</a><a href="#hotspots">hotspots</a><a href="#inventory">inventory</a><label for="theme">light / dark</label></div></nav></header>
<div class="hero" id="top"><div class="hero-box frame"><span class="eyebrow">STATIC ANALYSIS · PYTHON PRODUCTION SOURCE</span><h1>Complexity report</h1><p class="subtitle">The codebase is mostly made of small, low-complexity functions, but risk is concentrated in a few execution paths. <code>{html.escape(largest_module['path'])}</code> contains {largest_share:.1%} of production LOC; the peak is <code>{html.escape(peak['name'])}</code> at CC {peak['cc']}.</p><p class="meta">Snapshot <code>{commit[:12]}</code> · generated {created} · scope <code>src/**/*.py</code></p></div></div>
<main>
<section id="overview"><h2>General health</h2><div class="cards">
<div class="card frame"><div class="number">{total_loc:,}</div><p>physical source lines<br><strong>{total_sloc:,}</strong> nonblank/non-comment</p></div>
<div class="card frame"><div class="number">{len(callables)}</div><p>functions + methods<br><strong>{len(classes)}</strong> classes</p></div>
<div class="card frame"><div class="number">{avg_cc:.1f}</div><p>mean cyclomatic complexity<br><strong>{total_cc}</strong> aggregate decisions</p></div>
<div class="card frame"><div class="number">{high_count}</div><p>high-or-higher callables<br><strong>{high_count/len(callables):.1%}</strong> of callables</p></div>
</div></section>
<section><div class="summary"><article class="panel frame callout"><h3>Assessment: {peak_label} peak</h3><p>Most callables are easy to reason about, but the maximum CC of <strong>{peak['cc']}</strong> in <code>{html.escape(peak['name'])}</code> makes change risk uneven. Review high-complexity routines before adding more paths.</p><div class="bar" aria-label="Complexity distribution"><span class="a" style="width:{low_count/len(callables):.1%}">A {low_count}</span><span class="b" style="width:{moderate_count/len(callables):.1%}">B {moderate_count}</span><span class="c" style="width:{high_count/len(callables):.1%}">C+ {high_count}</span></div><p class="meta">{low_count} low (A), {moderate_count} moderate (B), {high_count} high or above (C–F).</p></article>
<article class="panel frame good"><h3>Testability signal</h3><p><strong>{test_count} test functions</strong> across {len(test_modules)} test modules and <strong>{test_loc:,} test LOC</strong> ({test_loc/total_loc:.2f}× production LOC). This is a useful safety net, but runtime coverage was not re-measured for this static report, so no coverage percentage is claimed.</p></article></div></section>
...
<section><h2>Priority actions</h2><div class="summary"><article class="panel frame"><h3>Review first</h3><ol>{review_items()}</ol></article><article class="panel frame"><h3>Do not optimize for size alone</h3><ul>{linear_size_items()}</ul></article></div></section>
